...
class StudentMetricsStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, construct_id: str, stage:str, **kwargs) -> None:
        ...
        student_bucket = bucket_stack.bucketStack(self, 'nested-id', 'student-metrics-dev')

        lambdas = ['course_month', 'most_popular', 'ranking_company']

        count = 0
        for lambda_name in lambdas:
            student_lambda = lambda_stack.lambdaStack(self, 'lambda-id-' + str(count), lambda_name=lambda_name, stage=stage)
            student_bucket.student_bucket.grant_read(student_lambda.student_lambda)
            count = count+1
        ...
        # student_bucket.student_bucket.grant_read(lambda_mostpopular)
        # student_bucket.student_bucket.grant_read(lambda_coursemonth)
        # student_bucket.student_bucket.grant_read(lambda_rankingcompany)
